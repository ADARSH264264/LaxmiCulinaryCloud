...
const { bannerImage, orbitImages, video } = bannerAssets;
  ...
  <div className="bg-gradient-to-br from-amber-900 via-amber-800 to-amber-700 text-white py-16 px-4 sm:px-8 relative overflow-hidden">
    <div className="absolute inset-0 bg-gradient-to-r from-amber-900/20 to-amber-700/18" />
    <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-12 relative z-10">
      ...
      <div className="flex-1 space-y-8 relative md:pr-8 lg:pr-10 text-center md:text-left">
        ...
        <p className="text-lg sm:text-xl md:text-lg lg:text-xl font-playfair italic text-amber-100">
  Best cooks and best delivery guys all at your service. Hot tasty food will reach you in 60 minutes.
</p>

<h1 className="max-w-xl opacity-90 mx-auto md:mx-0 text-3xl font-bold text-amber-200">
  Login
</h1>
...
<form onSubmit={handleSearch} className="relative max-w-2xl mx-auto md:mx-0 group">
  <div className="relative flex items-center bg-amber-900/30 rounded-xl border-2 border-amber-500 shadow-2xl hover:bg-amber-400/50 transition-all duration-300">
    <div className="pl-6 pr-3 py-4 flex items-center gap-2 w-full">
...
<div className="flex flex-wrap gap-4 justify-center md:justify-start mt-6">
  ...
  {/* Download App Button */}
  <button className="group flex items-center gap-3 bg-amber-800/30 hover:bg-amber-800/50 px-6 py-3 rounded-xl transition-all duration-300 border-2 border-amber-700/50 hover:border-amber-400">
    <FaDownload className="text-xl text-amber-400 group-hover:animate-bounce" />
    <span className="text-lg text-amber-100 font-medium">Download App</span>
  </button>
...
{orbitImages.map((imgSrc, index) => (
  <div
    ...
    className={`absolute left-1/2 top-1/2 
      ...
      w-[80px] xs:w-[100px] sm:w-[120px] 
      h-[80px] xs:h-[100px] sm:h-[120px]`}
  >
    <img
      src={imgSrc}
      alt={`Orbiting ${index + 1}`}
      className="w-full h-full rounded-full object-cover border-4 border-amber-800 shadow-md"
    />
  </div>
))}
...
{/* VIDEO MODAL */}
{showVideo && (
  <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/98 backdrop-blur-lg p-4">
    <button
