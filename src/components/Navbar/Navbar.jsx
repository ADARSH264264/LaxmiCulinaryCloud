...
const Navbar = () => {
  ...
  //  COMBAINE UPDATING LOGIN MODAL AND AUTH STATUS ON LOCATION CHANGE
...
const renderDesktopAuthButton = () => {
  return isAuthenticated ? (
    <button
      ...
      className="px-3 md:px-3 lg:px-6 py-1.5 md:py-2 lg:py-3 bg-gradient-to-br from-amber-500 to-amber-700 text-[#20180E] rounded-2xl font-bold hover:shadow-lg hover:shadow-amber-600/40 transition-all transform hover:scale-[1.02] border-2 border-amber-600/20 flex items-center space-x-2 shadow-md shadow-amber-900/20 text-xs md:text-sm lg:text-sm"
    >
 ...
 const renderMobileAuthButton = () => {
  return isAuthenticated ? (
    <button
      onClick={handleLogout}
      className="w-full px-4 py-3 bg-gradient-to-br from-amber-500 to-amber-700 text-[#201B0E] rounded-xl font-semibold flex items-center justify-center space-x-2 text-sm"
    >
      <FiLogOut />
      <span>Logout</span>
    </button>
    ...
    <button
      ...
      className="w-full px-4 py-3 bg-gradient-to-br from-amber-500 to-amber-700 text-[#2D180E] rounded-xl font-semibold flex items-center justify-center space-x-2 text-sm"
    >
      <FiKey />
      <span>Login</span>
    </button>
  ...
  return (
    <nav className="bg-[#2D1B0E] border-b-8 border-amber-900/30 shadow-[0_24px_50px_12px_rgba(0,0,0,0.3)] sticky top-0 z-50 font-vibes group/nav overflow-x-hidden">

      {/* Decorative Gradient and Icons */}
  ...
  {/* LOGO SECTION */}
<div className="flex items-center gap-2 group relative ml-0 md:ml-2 flex-nowrap min-w-0">
  <div className="absolute -inset-4 bg-amber-500/10 rounded-full blur-xl opacity-80 group-hover/nav:opacity-100 transition-opacity duration-300" />

  <GiChefToque className="text-3xl sm:text-4xl lg:text-5xl text-amber-500 transition-all group-hover:rotate-12 group-hover:text-amber-400 hover:drop-shadow-[0_0_15px_#F59E0B]" />

  <div className="flex flex-col relative ml-2 flex-shrink-0 overflow-visible">
    <NavLink
      to="/"
      className="text-[1.3rem] sm:text-2xl md:text-3xl lg:text-4xl bg-gradient-to-r from-amber-400 to-amber-600 bg-clip-text text-transparent font-monsieur tracking-wide drop-shadow-[0_2px_2px_black] whitespace-nowrap overflow-visible"
    >
      LAXMI-KITCHEN
    </NavLink>
    ...
    <div className="h-[3px] bg-gradient-to-r from-amber-600/30 via-amber-400/50 to-amber-600/30 mt-1 ml-1 shadow-[0_2px_5px] shadow-amber-500/20" />
            ...
            {navLinks.map((link) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  ...
                  ${isActive ? 'border-amber-600/50 bg-amber-900/20 shadow-[inset_0_0_15px]' : 'border-amber-900/30 hover:border-amber-600/50 shadow-md shadow-amber-900/20'}`
                }
              >
                <span className="mr-2 text-sm md:text-[15px] lg:text-base text-amber-500 group-hover:text-amber-300 transition-all">
                  {link.icon}
                </span>
                <span className="text-amber-100 group-hover:text-amber-300 relative shadow-amber-500/20">
                  {link.name}
          ...
          <div className="md:hidden flex items-center mr-2">
            ...
            >
              <div className="space-y-2 relative">
                <span className={`block w-6 h-[2px] bg-current transition-all duration-300 ${isOpen ? 'rotate-45 translate-y-[7px]' : ''}`} />
                <span className={`block w-6 h-[2px] bg-current transition-all duration-300 ${isOpen ? 'opacity-0' : 'opacity-100'}`} />
                <span className={`block w-6 h-[2px] bg-current transition-all duration-300 ${isOpen ? '-rotate-45 -translate-y-[7px]' : ''}`} />
              </div>
            </button>
          </div>
      ...
      {/* MOBILE NAVIGATION */}
      {isOpen && (
        <div className="md:hidden bg-[#20180E] border-t-4 border-amber-900/40 relative shadow-lg shadow-amber-900/30 w-full">
          <div className="px-4 py-4 space-y-2">
            {navLinks.map((link) => (
              <NavLink
                ...
                onClick={() => setIsOpen(false)}
                className={({ isActive }) =>
                  `block px-4 py-3 text-sm rounded-xl transition-all flex items-center border-2 ${isActive ? 'bg-amber-600/30 border-amber-600/50' : 'border-amber-900/30'}`
                }
              >
      ...
      {showLoginModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-gradient-to-br from-[#20180E] to-[#4a372a] rounded-xl p-6 w-full max-w-[480px] relative border-4 border-amber-700/30 shadow-[0_0_30px] shadow-amber-500/30">
            ...
            <Login onLoginSucess={handleLoginSucess} onClose={() => navigate('/')}>
