...
export const OurHomeMenu = () => {
 const [activeCategory, setActiveCategory] = useState('Breakfast');
 // ✅ local state for category , upadte  menu catergory
...
const displayItems = (dummyMenuData[activeCategory] || []).slice(0, 4);
 ...
 // ✅ custome  hook   data  coming  from  cart
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 sm:px-6 py-2 rounded-full border-2 transition-all duration-300 transform
                ...
                ${
                  activeCategory === cat
                    ? 'bg-gradient-to-r from-amber-900/80 to-amber-700/80 border-amber-800 scale-105 shadow-lg text-amber-100/90'
  ...
  {displayItems.map((item, i) => {
    ...
    return (
      <div
        ...
        style={{ '--index': i }}
      >
        <div className="relative h-48 sm:h-56 md:h-60 flex items-center justify-center bg-black/18">
          <img
            src={item.image}
            alt={item.name}
