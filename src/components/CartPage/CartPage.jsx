...
export const CartPage = () => {
  ...
  <div className="text-center animate-fade-in">
    ...
    <Link
      ...
      gap-2 hover:gap-3 hover:bg-amber-800/50 bg-amber-900/40 px-6 py-2 rounded-full 
      font-cinzel text-sm uppercase"
    >
      Browse All Items
    </Link>
  </div>
  ...
  <div key={item.id}>
    <div
      ...
      transition-all duration-300 hover:border-solid hover:shadow-xl 
      hover:shadow-amber-900/10 transform hover:-translate-y-1 animate-fade-in"
    >
      <div
      ...
      {/* 👇 Move this quantity section INSIDE the item box */}
...
<div className="mt-12 pt-8 border-t border-amber-800/30 animate-fade-in-up">
  <div className="flex flex-col sm:flex-row justify-between items-center gap-8">
    ...
    <Link
      to="/menu"
      className="bg-amber-900/40 px-8 py-3 rounded-full font-cinzel uppercase tracking-wider 
      hover:bg-amber-800/50 transition-all duration-300 text-amber-100 inline-flex 
    ...
    <div className="flex flex-col sm:flex-row items-center gap-6">
      ...
      <button
        className="bg-amber-900/40 px-8 py-3 rounded-full font-cinzel uppercase tracking-wider 
        hover:bg-amber-800/50 transition-all duration-300 text-amber-100 flex 
...
{selectedImage && (
  <div
    ...
    bg-amber-900/40 bg-opacity-75 backdrop-blur-sm p-4 overflow-auto"
    onClick={() => setSelectedImage(null)}
  >
    <div
      className="relative max-w-full max-h-full"
      onClick={(e) => e.stopPropagation()} // Image pe click karne se modal band na ho
    >
      <img
        src={selectedImage}
        alt="Full View"
      ...
      <button
        onClick={() => setSelectedImage(null)}
        className="absolute top-2 right-2 bg-amber-900/80 rounded-full p-2 
        text-amber-100 hover:bg-amber-800 transition duration-200 active:scale-90"
      >
